...
sudo mkdir -p /var/www/webapp/royalcargo/{current,backups,temp,logs}
sudo chown -R $USER:$USER /var/www/webapp/royalcargo

# Rendre le script de déploiement exécutable
echo "🔧 Configuration des permissions..."
chmod +x /var/www/webapp/royalcargo/deploy.sh
...
sudo tee /etc/nginx/sites-available/royalcargo << EOF
server {
    listen 80;
    server_name royalcargo225.com; # Remplacez par votre domaine
...
# Activer le site
sudo ln -s /etc/nginx/sites-available/royalcargo /etc/nginx/sites-enabled/
sudo nginx -t
