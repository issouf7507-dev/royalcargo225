...
import { delUser, postNewImage, putAdress } from "@/app/api";
...
export const columns: ColumnDef<DataUsing>[] = [
...

  {
    ...
    cell: ({ row }) => {
      ...
      // const sendSms = async (phone: string, mess: string) => {
      //   try {
      //     const res = await fetch("/api/bulksms", {
      //       method: "POST",
      //       headers: {
      ...
      function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        formData.append("imagecolis", image);

        putAdress(values, token).then((res) => {
          if (res) {
            refetch();
          ...
          sendSMS(
            ...
            `ROYAL CARGO \nBONJOUR CHER CLIENT (${form.watch(
              ...
              .watch("etat")
              .toUpperCase()}**.\nPOUR PLUS DE DÉTAILS RENDEZ-VOUS sur royalcargor225.com AVEC VOTRE NUMÉRO DE SUIVI: ${form.watch(
              "trackCode"
            )}`
          );
          // sendSms(
          //   form.watch("numero"),
          //   `ROYAL CARGO \nBONJOUR CHER UTILISATEUR. NOUS SOMMES RAVIS DE VOUS ANNONCER QUE LE STATUT DE VOTRE COLIS À CHANGER, IL EST MAINTENANT PASSER À **${form.watch(
          //     "etat"
          //   )}**.\nPOUR PLUS DE DÉTAILS RENDEZ-VOUS sur royalcargor225.com AVEC VOTRE NUMÉRO DE SUIVI: ${form.watch(
          //     "trackCode"
          //   )}`
        ...
        <div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              ...
              >
                <Pencil className="mr-2" /> Mettre a jour
              </Button>
            </DialogTrigger>
